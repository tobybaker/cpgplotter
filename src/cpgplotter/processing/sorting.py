...
def _sort_by_clustering(
    methylation_matrix: np.ndarray, read_names: np.ndarray
    ...
    """
    indices = hierarchical_cluster_reads(methylation_matrix)
...
def _sort_by_qualitative_and_cluster(
    ...
    for group_key, group_df in sorted_annot.group_by(
        ...
        clustered_indices = hierarchical_cluster_reads(
            methylation_matrix, read_indices=group_indices
        )
...
def sort_reads(
    ...
    if annotations is None or sort_by is None or len(sort_by) == 0:
        return _sort_by_clustering(methylation_matrix, read_names)
...
def hierarchical_cluster_reads(
    ...
    Uses Hellinger distance between methylation probability vectors for CpGs
    covered by both reads. For reads with no overlapping CpGs, uses a large
    fixed distance.
    ...
    # Compute Hellinger distance for reads
    distances = compute_hellinger_distance_matrix(subset)
...
def compute_hellinger_distance_matrix(methylation_matrix: np.ndarray) -> np.ndarray:
    """
    Compute pairwise Hellinger distances between reads.

    Hellinger distance is computed only for CpGs covered by both reads.
    For read pairs with no overlapping CpGs, assigns the maximum Hellinger
    distance (sqrt(2)).
    ...
    """
    return _compute_hellinger_distances_numba(
        np.ascontiguousarray(methylation_matrix, dtype=np.float64)
    )


@numba.njit
def _compute_hellinger_distances_numba(matrix: np.ndarray) -> np.ndarray:
    """Numba-accelerated pairwise Hellinger distance computation."""
    n_reads = matrix.shape[0]
    n_cpgs = matrix.shape[1]
    no_overlap_distance = np.sqrt(2.0)
    ...
    for i in range(n_reads):
        for j in range(i + 1, n_reads):
            ...
            if n_overlap > 0:
                distances[idx] = np.sqrt(0.5 * sum_sq)
            else:
                distances[idx] = no_overlap_distance
